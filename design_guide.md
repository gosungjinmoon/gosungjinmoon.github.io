...
| **폰트** | **Inter (권장)**, *Montserrat*, *Poppins Rounded* (원형 마침표 지원 폰트 우선) |
| **심볼(빛)** | 워드마크 위의 짧은 3줄의 옐로우 레이(Ray) — 영감과 아이디어의 순간 (최종 SVG 코드 기반) |
...
### 📐 사용 규칙
  ...
  * 어두운 배경(`Background (Dark)`)에는 **화이트/크림 텍스트 + 옐로우 심볼** 조합을 사용합니다.
...
:root {
  ...
  /* 다크 모드 시 재정의 */
  --dark-bg: #1A3258;
  --dark-fg: #F0E5D4; /* [업데이트] Light Gray(#F3F4F6)에서 Creamy Beige(#F0E5D4)로 변경 (SVG와 통일) */
  --dark-card: #1E2A3A;
}
...
body.theme-typo {
  ...
  color: var(--color-fg);
  /* [업데이트] 국문 Pretendard, 영문 및 로고 Inter, Montserrat 우선 */
  font-family: 'Inter', 'Montserrat', 'Poppins Rounded', 'Pretendard', sans-serif;
}
...
### 📐 사용 규칙
  ...
  * 심볼 단독 사용 가능 (예: 앱 아이콘 / 정사각 1:1 비율)
...
| **ICO / SVG Favicon** | 브라우저 탭, 북마크용. **(Typo 로고의 빛 심볼 사용 권장)** |
...
/assets/logo/
  ...
  ├── favicon.svg              (Typo 심볼, 투명)
  ├── apple-touch-icon.svg   (Typo 심볼, 단색 배경)
```
