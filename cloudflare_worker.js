/* cloudflare_worker.js  버전 정보_202510171345 */
export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname === "/oauth-url") {
      const client_id = env.GITHUB_CLIENT_ID || "YOUR_CLIENT_ID";
      const redirect_uri = env.GITHUB_REDIRECT_URI || "https://YOUR_DOMAIN/admin/oauth/callback.html";
      const scope = "repo";
      const authorize_url = `https://github.com/login/oauth/authorize?client_id=${client_id}&redirect_uri=${encodeURIComponent(redirect_uri)}&scope=${scope}`;
    ...
    if (url.pathname === "/api/new-post" && request.method === "POST") {
      try {
        ...
        const repo = env.GITHUB_REPO;GITHUB_TOKEN
        const token = env.GITHUB_TOKEN;
        const base = env.DEFAULT_BASE_BRANCH || "main";
        if (!owner || !repo || !token) throw new Error("Worker env missing GITHUB_* vars");
        const gh = async (api, init = {}) => {
          const res = await fetch(`https://api.github.com${api}`, {
            ...init, headers: { "content-type":"application/json","authorization":`token ${token}`,"accept":"application/vnd.github+json",...(init.headers||{}) }
          });
          if (!res.ok) throw new Error(await res.text()); return res.json();
        };
        const baseRef = await gh(`/repos/${owner}/${repo}/git/ref/heads/${base}`); const baseSha = baseRef.object.sha;
        const branch = `post-${Date.now()}`; await gh(`/repos/${owner}/${repo}/git/refs`, { method:"POST", body: JSON.stringify({ ref:`refs/heads/${branch}`, sha:baseSha }) });
        ...
        await fetch(`https://api.github.com/repos/${owner}/${repo}/git/refs/heads/${branch}`, { method:"PATCH", headers:{ "content-type":"application/json","authorization":`token ${token}` }, body: JSON.stringify({ sha: commit.sha, force: true }) });
